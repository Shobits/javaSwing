...
import org.example.vo.CommonResult;
...
public class ProvinceCityController {
    ...
    public CommonResult queryProvinces() {
        ...
        CommonResult cr = new CommonResult(1, "查询失败", "");
    ...
    public CommonResult addCity(City city) {
        ...
        if (rows > 0) {
            ...
            cr.setCode(1);
            cr.setMsg("添加城市 [ " + city.getName() + " ] 失败");
            cr.setData("");
        }
        return cr;
    }
    ...
    public CommonResult queryCity(Integer pid) {
        ...
        CommonResult cr = new CommonResult(1, "查询无结果", "");
        if (cityList != null && cityList.size() > 0) {
            cr = new CommonResult(0, "查询出 [ " + cityList.size() + " ] 个城市", cityList);
        }
        return cr;
    }
}
